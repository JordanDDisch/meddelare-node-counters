...
function MeddelareCounters(options) {
    ...
    // Set credentials separately instead of relying on the deep copy.
...
MeddelareCounters.prototype._networkCallbacks = {
    ...
    facebook: function(url, callback) {
        ...
        var apiUrl = "https://graph.facebook.com/?id=" + encodeURIComponent(url);

        if(this._options.credentials.facebook) {
            var credentials = this._options.credentials;
            if (credentials.facebook.appId && credentials.facebook.appSecret) {
                apiUrl += '&access_token=' + credentials.facebook.appId + '|' + credentials.facebook.appSecret;
                console.log('Used access_token to make request');
            }
        } else {
            console.log('Did not use access_token to make request');
        }
